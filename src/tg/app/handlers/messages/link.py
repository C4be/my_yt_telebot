...
async def links_handler(message: Message) -> None:
    try:
        ...
        logger.warning(f"Ошибка валидации ссылки от пользователя {message.from_user.id}: {e}")
        return
    
    logger.info(f"Получена корректная ссылка от {message.from_user.username or message.from_user.id}: {link_data.link}")
