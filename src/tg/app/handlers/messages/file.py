...
@router.message(
    ...
    | (
        F.forward_from.as_("forwarded")
        & F.document
        & F.document.file_name.regexp(r"\.txt$")
    )
    | (
        F.reply_to_message
        & F.reply_to_message.document
        & F.reply_to_message.document.file_name.regexp(r"\.txt$")
    )
)
async def file_handler(message: Message):
    ...
    logger.info(
        f"Получен файл {file_name} от пользователя {message.from_user.username or message.from_user.id}"
    )
